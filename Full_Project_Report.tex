...
\emph{RoBERTa shows the cleanest diagonal, confirming its superior
ability to handle complex social media nuances and sarcastic tones.}
...
\emph{The multilingual variant demonstrates similar patterns to RoBERTa,
suggesting that its broad language training remains highly effective for
English sentiment.}
...
\emph{BERT Base maintains high accuracy for negative sentiment detection
but shows slight confusion with neutral posts that use descriptive
language.}
...
\emph{Logistic Regression displays surprisingly sharp classification for
a keyword-based approach, though it lacks the contextual depth of
Transformers.}
...
\emph{SVM shows a stable distribution but struggles more with neutral
samples compared to the high-performing Transformer models.}
...
\emph{Naive Bayes serves as a solid baseline, though its matrix reveals
more frequent misclassifications due to its ``bag-of-words''
...
\item
  \textbf{Advanced Filtering:} Users can filter results by platform,
  date range, and sentiment strength.
\end{itemize}
...
Below are screenshots showcasing the user interface and the interactive
visualizations.
