...
The research question covers a decade of social media data. If the date
range is narrower than expected, or if any dates are invalid (e.g.
...
The p-value of **0.08 sits above the conventional alfa = 0.05 threshold**,
so we formally fail to reject the null hypothesis of balance. However,
this is a **borderline result** that deserves honest reporting: with
only 326 posts after deduplication, the test has limited statistical
power. The descriptive picture — neutral at \~38.7% versus negative at
\~28.5% — reveals a mild practical imbalance that will be relevant
context for any accuracy metric computed in Phase 2.
...
## Sentiment $\times$ Platform

The structural affordances of each platform — Facebook's long-form
posts, Instagram's caption-driven visual format, and Twitter's
...
Phase 3 dashboard should only be presented as analytically meaningful if
...
cat("  $\\chi^2$(", chisq_platform$parameter, ") =", round(chisq_platform$statistic, 3), "\n")
...
With \$\\chi\^2\$(4) = 3.285, p = 0.51, and Cramér's V = 0.071, **the
...
itself a finding: it tells Emma Martin that the *medium does not shape
the emotional message* in this dataset, and it means that platform
...
This is the the client's core research question, *"how has sentiment on social networks evolved overthe last decade?"*, is answered directly here. **Proportions** are plotted rather than raw counts to correct for the unequal number of posts per year: a year with 5 posts and a year with 50 posts are not directly comparable on a raw-count axis.
...
ggplot(yearly_sentiment, aes(x = Year, y = pct, colour = sentiment, group = sentiment)) +
  ...
  labs(
    title    = "Historical Sentiment Trend on Social Media (2010–2023)",
...
ggplot(monthly_heat, aes(x = factor(Month, labels = month.abb), y = Platform, fill = pct)) +
  ...
  geom_text(aes(label = paste0(round(pct * 100, 1), "%")), size = 3) +
