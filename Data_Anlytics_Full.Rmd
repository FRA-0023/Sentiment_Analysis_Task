...
These models utilize the self-attention mechanism to capture deep contextual relationships within the text, moving beyond simple word-matching to understand the "meaning" of sentences.

1.  **RoBERTa (roberta-base):** 
    *   **Architecture:** RoBERTa (Robustly Optimized BERT Approach) is an evolution of the original BERT model. It was trained using a significantly larger dataset (160GB of text vs. 16GB for BERT) and for a longer duration.
    *   **Key Innovation:** It removes the "Next Sentence Prediction" task from BERT, focusing entirely on "Masked Language Modeling." This allows it to learn more robust linguistic representations.
    *   **Performance in this Project:** It consistently achieved the highest accuracy (86.67%) because of its superior ability to handle the subtle nuances and sarcasm often found in social media posts.

2.  **BERT (bert-base-uncased):** 
    *   **Architecture:** The pioneer of bidirectional Transformers. "Uncased" means it treats "Happy" and "happy" identically, reducing the vocabulary size and focusing on semantic content.
    *   **Working Principle:** Unlike previous models that read text left-to-right or right-to-left, BERT reads the entire sequence of words at once. This allows it to understand the context of a word based on all of its surroundings.
    *   **Role in Project:** Served as the primary benchmark for contextual models.

3.  **Multilingual BERT (bert-base-multilingual-uncased):** 
    *   **Architecture:** Trained on the top 104 languages with the largest Wikipedias. 
    *   **Capability:** It shares a common vocabulary across all languages, enabling "zero-shot" transfer where it can understand concepts in one language even if it was primarily trained on another.
    *   **Significance:** Included to ensure the pipeline is robust to the multi-lingual slang and loanwords common in Italian/French social media contexts.

4.  **DistilBERT (distilbert-base-uncased):** 
    *   **Architecture:** A "distilled" version of BERT. It uses a technique called "Knowledge Distillation" where a smaller "student" model (DistilBERT) is trained to mimic the behavior of a larger "teacher" model (BERT).
    *   **Trade-off:** It is 40% smaller and 60% faster than BERT while retaining 97% of its language understanding capabilities.
    *   **Observed Behavior:** In our specific dataset, it struggled slightly more with "Neutral" sentiment, showing that for very short social media posts, the full complexity of BERT/RoBERTa is often necessary.

### 9.2.2 Statistical Classifiers (Machine Learning)

These models rely on frequency-based feature extraction (TF-IDF). They treat text as a "bag of words," counting how often specific terms appear.

5.  **Logistic Regression:** 
    *   **Methodology:** A linear model that uses a logistic function (sigmoid) to model the probability of a post belonging to a specific sentiment class.
    *   **Strengths:** Extremely fast and highly interpretable. It assigns a specific "weight" to each word, allowing us to see which words (like "sad," "great," or "wait") are the strongest predictors of sentiment.
    *   **Project Finding:** Surprised the team by outperforming DistilBERT, proving that for this dataset, simple keyword presence is a very powerful signal.

6.  **Support Vector Machine (SVM):** 
    *   **Methodology:** Works by finding the "Optimal Separating Hyperplane"—a mathematical boundary that maximizes the distance between different sentiment classes in a high-dimensional space.
    *   **Kernel:** We utilized a Linear Kernel, which is highly effective for high-dimensional text data where classes are often linearly separable.
    *   **Reliability:** Known for being robust against overfitting, especially in cases where the number of features (words) exceeds the number of samples.

7.  **Multinomial Naive Bayes:** 
    *   **Methodology:** Based on Bayes' Theorem, it calculates the probability of a sentiment given a set of words. It assumes that every word is "independent" of every other word (the "naive" assumption).
    *   **Utility:** Despite its simplicity, it is the industry standard for baseline text classification. It is incredibly efficient to train and served as the project's performance "floor."
...
*RoBERTa shows the cleanest diagonal, confirming its superior ability to handle complex social media nuances and sarcastic tones.*
...
*The multilingual variant demonstrates similar patterns to RoBERTa, suggesting that its broad language training remains highly effective for English sentiment.*
...
*BERT Base maintains high accuracy for negative sentiment detection but shows slight confusion with neutral posts that use descriptive language.*
...
*Logistic Regression displays surprisingly sharp classification for a keyword-based approach, though it lacks the contextual depth of Transformers.*
...
*SVM shows a stable distribution but struggles more with neutral samples compared to the high-performing Transformer models.*
...
*Naive Bayes serves as a solid baseline, though its matrix reveals more frequent misclassifications due to its "bag-of-words" independence assumption.*
...
- **Advanced Filtering:** Users can filter results by platform, date range, and sentiment strength.

## 11.2 Website Link & Interface
The dashboard is hosted at the following link:
**[Link to Dashboard: <https://jemin-sentiment-dashboard.lovable.app/>]**

### Dashboard Screenshots
Below are screenshots showcasing the user interface and the interactive visualizations.
 
```{r dashboard-overview, out.width="100%", fig.cap="Figure 6: Overview of the Sentiment Distribution Dashboard", echo=FALSE}
knitr::include_graphics("Model/Result/dashboard_overview.png")
```

```{r dashboard-analysis, out.width="100%", fig.cap="Figure 7: Historical Trends and Platform-specific Analysis", echo=FALSE}
knitr::include_graphics("Model/Result/dashboard_analysis.png")
```
